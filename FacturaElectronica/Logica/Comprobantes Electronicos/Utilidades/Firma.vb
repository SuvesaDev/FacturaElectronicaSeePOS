...
Public Class Firma
    ...
    Public Sub FirmaXML_Xades(pathXML As String, certificado As String, esThumbprint As Boolean, clave As String)
        Try
            ...
            If esThumbprint Then
                '956EE337A337CD4D4FA73D2E536F40B431A1EDB7
                cert = GetCertificateByThumbprint(certificado)
            Else
                cert = New X509Certificate2(certificado, clave)
            End If

            Me.ValidarCertificado(certificado, esThumbprint, clave)
    ...
    Public Sub FirmaXML_Xades_MensajeReceptor(pathXML As String, certificado As String, esThumbprint As Boolean, clave As String)
        Try
            Dim cert As X509Certificate2
    ...
    Public Function ValidarCertificado(certificado As String, esThumbprint As Boolean, clave As String, Optional _NotificarValidacion As Boolean = False) As Boolean
        Try
            Dim cert As X509Certificate2
            ...
            General.sql.Ejecutar("update Seguridad.dbo.Emisor set VenceCertificado = '" & cert.GetExpirationDateString & "'", CommandType.Text)
            Return True
        Catch ex As Exception
            MsgBox(ex.Message, MsgBoxStyle.Exclamation, "Certificado")
            Return True
        End Try
    End Function
